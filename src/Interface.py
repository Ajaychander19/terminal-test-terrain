...
class GUI(tkinter.Frame):
    ...
    def __init__(self,master=None,**kwargs):
        ...
        self.lnameLabel = tkinter.Label(self, text="COVERAGE OF 4G BASE STATIONS",font = boldFont,background='light blue')
        self.lnameLabel.pack(padx=5, pady=5)

        self.defaultPath = tkinter.Button(self, command=lambda: self.buttonClick(1), text="Configuration",font = boldFont,background='light green')
        self.defaultPath.configure(height=3,width=25)
        self.defaultPath.pack(padx=5, pady=5)

        self.pcap_conversion = tkinter.Button(self, command=lambda: self.buttonClick(2), text="Field-test *.csv to *.pcap \n and *.json conversion",
        ...
        self.pcap_conversion_ttp = CreateToolTip(self.pcap_conversion,
                                               "Choosing the Field-test file from the ZkSamp for generating Field-test json files as well as the pcap files corresponde to operators")

        self.cartoradio_files = tkinter.Button(self, command=lambda: self.buttonClick(3),
                                              text="Cartoradio File Conversion",font = boldFont,background='light green')
        self.cartoradio_files.configure(height=3, width=25)
        self.cartoradio_files.pack(padx=5, pady=5)
        self.cartoradio_files_ttp = CreateToolTip(self.cartoradio_files,
                                               "Choosing two csv files from the cartoradio for generating a new csv file with all the information needed ")

        self.association = tkinter.Button(self, command=lambda: self.buttonClick(4),
        ...
        self.association_ttp = CreateToolTip(self.association,
                                                  "Choose the .csv site file created from the 'Cartoradio File Conversion' and the json field-test files")

        self.visualizaion = tkinter.Button(self, command=lambda: self.buttonClick(5),
    ...
    def buttonClick(self,number):
        ...
        if number == 1:   # Selecting output directory.
            ...
            files = filedialog.askopenfilenames(initialdir=self.working_directory, title='Choose a file')
            if (len(files)<2):
                messagebox.showinfo("Warning", "There are problems on selected files ")
            else:
            ...
            files = filedialog.askopenfilenames(initialdir=self.working_directory, title='Choose a file')
            if (len(files)>=2):
                cell = Associate_cell(files,self.working_directory)    #dans
                if cell == "ERROR":
